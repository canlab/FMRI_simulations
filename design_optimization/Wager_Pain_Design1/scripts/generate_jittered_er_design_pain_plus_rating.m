function [meanrecipvif, vifs, design_struct] = generate_jittered_er_design_pain_plus_rating(varargin)
...
event1duration = 8;    % duration of stim event
event2duration = 5;    % duration of rating event
trialtypes = 6;        % 2 (body site) x 3 (stim type)
trialspertype = 8;
...
ISImean = 5;            % For 'exponential' only.  Includes ISImin.  There is an optimal empirical value -- longer is better for deconvolution/FIR, but we also need to fit within total scan time constraints.
...
ISImax = 16;            % Truncate to avoid VERY long ISIs
...
% Parse structure inputs if entered
% Assign fields to variables in workspace, overwriting defaults
% -----------------------------------
wh = cellfun(@isstruct, varargin);
if any(wh)
    IN = varargin(wh); % cell with struct inside
    ...
    for i = 1:length(IN)
        ...
        for j = 1:length(fn)
            str = sprintf('%s = IN{i}.(''%s'');', fn{j}, fn{j});
            eval(str)
        end
...
for i = 1:length(varargin)
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case {'contrasts', 'contrastweights'} % do nothing, handle later after we know trial types
...
% each trial (stim), not the second (ratings). Add zeros for 2nd event
% within trial, and intercept
contrasts = [contrasts zeros(trialtypes, trialtypes + 1)];

if calc_efficiency

    xc = load('myscannerxc.mat');
    [S,~,svi] = getSmoothing(design_struct.HPlength, 0, design_struct.TR, design_struct.scanlength ./ TR, xc.myscannerxc);
    
    [design_struct.efficiency, design_struct.eff_vector] = calcEfficiency(contrastweights,contrasts,pinv(X),svi);

...
pthr = (1 - tcdf(5.5, N)) ./ 2 % typical corrected threshold, use for sim 2
...
% P < .001, N = 30 : power = 0.08 - 1.1%

% max number of samples replicating a voxel
num_samples_sig = sum(is_sig_matrix');
max(num_samples_sig)  % 3

% mean number of samples replicating a true voxel
num_samples_sig = sum(is_sig_matrix(istrue, :)');
mean(num_samples_sig)  % .12

% At a typical threshold for P < .05 FWER (P < 1.42 x 10-6, t = 5.5) and N = 30, 
% with an effect size of d = 0.5 in 19,532 voxels,
% the average true-positive voxel is significant in only .12/25 or <1% of studies.
% No voxel in the brain is identified by more than 3/25 or about 12% of studies.
...
mkdir(figsavedir)
saveas(gcf, fullfile(figsavedir, 'power_fpr_maps1_p05corr_n30.svg'))
...
saveas(gcf, fullfile(figsavedir, 'power_fpr_maps1_p05corr_n30_notrueregions.svg'))
